...
| Writing AKAI **disk images** | ⚠️ **built, and unreleased.** Hard disk, CD3000 disc and 1.6 MB floppy, plus append. Verified only in the sense that matters least: our own reader gets back byte-for-byte what our writer put in (44 files across two volumes, per medium), and `akaiutil` agrees on the result. **No S3000XL has mounted any of it.** It stays on an unmerged branch for that reason, and behind `check_akai_write_support()` so it cannot appear on a checkout without mpc2emu's AKAI writer — a merge alone would not surface it |
