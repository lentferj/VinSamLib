...
| Writing AKAI **disk images** | ⚠️ **not offered**, and an AKAI bank is refused by **Send to Image Column** up front rather than at the end of a build. mpc2emu can build them and its output is byte-identical to `akaiutil`'s, but no S3000XL has been asked to mount one. Until that happens, VinSamLib will not put media in front of you that a sampler might refuse. The New Bank folder above is the way out in the meantime |
