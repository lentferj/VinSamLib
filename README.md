...
| Writing AKAI **disk images** | ⚠️ **built, and unreleased.** Hard disk, CD3000 disc and 1.6 MB floppy, plus append. Verified only in the sense that matters least: our own reader gets back byte-for-byte what our writer put in (44 files across two volumes, per medium), and `akaiutil` agrees on the result. **No S3000XL has mounted any of it.** It stays on an unmerged branch for that reason, and behind `check_akai_write_support()`. **That gate got weaker on 2026-09-07**: mpc2emu merged its own `akai-s3000xl` branch into `main`, so an ordinary checkout now HAS the AKAI writer and the capability check passes on it. Until that merge, a merge on this side alone would not have surfaced the feature; now it would. The branch is the only thing holding it back |
